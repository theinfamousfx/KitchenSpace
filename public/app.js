// ============================================================================
// COMPLETE app.js - MODALS CENTERED PROPERLY
// ============================================================================
...
function showForm(type, item = null) {
  ...
  if (type === 'recipe') {
    formHtml += `<input type="text" placeholder="Title *" name="title" value="${item?.title || ''}" required>`;
    formHtml += `<input type="text" placeholder="Cuisine" name="cuisine" value="${item?.cuisine || ''}">`;
    formHtml += `<input type="text" placeholder="Difficulty" name="difficulty" value="${item?.difficulty || ''}">`;
    formHtml += `<textarea placeholder="Description" name="description">${item?.description || ''}</textarea>`;
    formHtml += `<textarea placeholder="Ingredients" name="ingredients">${item?.ingredients || ''}</textarea>`;
    formHtml += `<textarea placeholder="Instructions" name="instructions">${item?.instructions || ''}</textarea>`;
  } else if (type === 'sop') {
    formHtml += `<input type="text" placeholder="Title *" name="title" value="${item?.title || ''}" required>`;
    formHtml += `<input type="text" placeholder="Category" name="category" value="${item?.category || ''}">`;
    formHtml += `<input type="text" placeholder="Priority" name="priority" value="${item?.priority || ''}">`;
    formHtml += `<textarea placeholder="Description" name="description">${item?.description || ''}</textarea>`;
    formHtml += `<textarea placeholder="Steps" name="steps">${item?.steps || ''}</textarea>`;
    formHtml += `<textarea placeholder="Compliance Notes" name="compliancenotes">${item?.compliancenotes || ''}</textarea>`;
    ...
    formHtml += `<input type="text" placeholder="Title" name="title" value="${item?.title || ''}">`;
    formHtml += `<input type="text" placeholder="Category" name="category" value="${item?.category || ''}">`;
    formHtml += `<textarea placeholder="Content *" name="content" required>${item?.content || ''}</textarea>`;
  } else if (type === 'video') {
    formHtml += `<input type="text" placeholder="Title *" name="title" value="${item?.title || ''}" required>`;
    formHtml += `<input type="url" placeholder="URL *" name="url" value="${item?.url || ''}" required>`;
    formHtml += `<input type="text" placeholder="Category" name="category" value="${item?.category || ''}">`;
    formHtml += `<textarea placeholder="Description" name="description">${item?.description || ''}</textarea>`;
  } else if (type === 'link') {
    formHtml += `<input type="text" placeholder="Title *" name="title" value="${item?.title || ''}" required>`;
    ...
    formHtml += `<input type="text" placeholder="Filename *" name="filename" value="${item?.filename || ''}" required>`;
    formHtml += `<input type="text" placeholder="Type" name="media_type" value="${item?.media_type || ''}">`;
    formHtml += `<input type="text" placeholder="Category" name="category" value="${item?.category || ''}">`;
    formHtml += `<textarea placeholder="Description" name="description">${item?.description || ''}</textarea>`;
  } else if (type === 'cookbook') {
    formHtml += `<input type="text" placeholder="Title *" name="title" value="${item?.title || ''}" required>`;
    formHtml += `<input type="text" placeholder="Author" name="author" value="${item?.author || ''}">`;
    formHtml += `<input type="text" placeholder="Cuisine" name="cuisine_type" value="${item?.cuisine_type || ''}">`;
    formHtml += `<textarea placeholder="Description" name="description">${item?.description || ''}</textarea>`;
  }

  formHtml += `<div class="form-buttons"><button type="submit" class="btn-save">Save</button><button type="button" class="btn-cancel" onclick="closeForm()">Cancel</button></div>`;
  formHtml += `</form>`;
    ...
    <div id="modal-backdrop" class="modal-backdrop">
      <div class="modal-box">
        <div class="modal-header">
          ...
          <button type="button" class="modal-close-btn" onclick="closeForm()">&times;</button>
        </div>
